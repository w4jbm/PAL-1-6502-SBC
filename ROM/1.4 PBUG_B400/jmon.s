; PBUG - A monitor program for the PAL-1 6502 Computer
;
; Based heavily on JMON which is
; Copyright (C) 2012-2021 by Jeff Tranter <tranter@pobox.com>
;
; Modification for the PAL-1 clone of the KIM-1 are
; Copyright (c) 2021 by Jim McClanahan W4JBM
;
; $ make pbug.bin
; $ srec_cat pbug.bin -binary -offset 0x???? -o pbug.hex -intel -CRLF
;
; Note: Use offset of 0x2000 (RAM) or 0xB400 (ROM)
;
;
...
T2      = $31                 ; Second byte (was previously no on Page 0)
...
VECTOR  = $3A                 ; Holds adddress of IRQ/BREAK entry point (2 bytes)
BPA     = $40                 ; Address of breakpoint (2 bytes * 4 breakpoints)
T3      = $3C                 ; Temp variable 3 (1 byte)
T4      = $3D                 ; Temp variable 4 (2 bytes)
...
; NEW NOTE: Working around ECHO issue by manipulating RIOT mask
  ECHO    = 1
...
; Get first character of command
        JSR GetKey
        CMP #13           ; Don't treat CR as an input error
        BNE _ckcmd
        JSR PrintCR
        JMP MainLoop
...
Invalid:
        JSR Imprint
        .byte CR,LF,"Invalid command. Type '?' for help.", CR, LF, 0
        RTS
...
; Copy Memory
;
; There are to copy routines.
;
; Copy Up:
; If SOURCE > DESTINATION, then start at the bottom of DESTINATION
; and copy up to avoid overwritting data you want to copy.
;
; Copy Down:
; If SOURCE < DESTINATION, then start at the top of DESTINATION and
; copy down to avoid overwritting data you want to copy.
;
;
; There is also a check that the END portion of the SOURCE address
; is equal to or greater than the START portion of the SOURCE
; address.
...
; This code is used for when the SOURCE>DESTINATION

@okayUp:
;        LDA #'U'    ; for troubleshooting
;        JSR PrintChar
        LDY #0
@copyUp:
...
; Increment SOURCE address (16 bits)

; This has been reworked and simplified. We have a 16 bit
; value in X and X+1 that we want to increment. So we
; increment X first and then check if it went to Zero.
; If not we are done, but if it did go from $FF to $00
; then we need to increment X+1.
...
; Increment DESTINATION address (16 bits)
        INC DL
        BNE @NoInc2
        INC DH
@NoInc2:
...
; Calculate the LENGTH of the copy
; LENGTH = SOURCE END (EL & EH) - SOURCE START (SL & SH)

        SEC
        LDA EL                 ; End (low)Calculate length = End - Start
        SBC SL
        STA T1
...
; Add LENGTH of copy to the DESTINATION address because
; we will put data at the destination from the top down
...
; This has been reworked and simplified. We have a 16 bit
; value in X and X+1 that we want to decrement. So we
; LDA X and see if it is Zero. If it is, we need to decrement
; both X+1 and X. Otherwise we just need to decrement X.
...
.elseif .defined(OSI)
;        .byte "Testing memory from", CR, LF, "$", 0
.endif
;        LDX START
;        LDY START+1
;        JSR PrintAddress
...
;        JSR PrintAddress
;       JSR Imprint
;       .byte CR,LF,"Press any key to stop", CR, LF, 0
;        JMP MEM_TEST
...
VNOTINRAM:
        JSR Imprint
        .byte "BRK vector not in RAM!", CR, LF, 0
        RTS
BNOTINRAM:
        JSR Imprint
        .byte "Breakpoint not in RAM!", CR, LF, 0
        RTS
...
BPREMOVE:
        ...
        BNE OK
        JSR Imprint
        .byte "Breakpoint not set!", CR, LF, 0
        PLA
        RTS
OK:
        PLA
...
;       JSR PrintCR
        ...
        STA RETOK               ; Set flag to accep return
writeLoop:
        JSR PrintSpace          ; Echo space
        JSR GetByte             ; Get data byte (ESC will exit)
        BCC @noret              ; Hopefully return will exit also W4JBM
        JSR PrintCR
        RTS
@noret: LDY #0
...
;
; Modified to Intel Hex by W4JBM
...
validType:
        ...
        sta     checksum        ; and use for start of checksum
...
getType:
        jsr     getHexByte      ; Get type
        sta     recordType     ; Save as byte count
        cmp     #0              ; Record Type 0 has data
        beq     checkcnt
        cmp     #1              ; Record Type 1 means we are done
        beq     endofload
        bne     invalidRecord   ; Anything else is an error

checkcnt:
        lda     byteCount
        cmp     #0              ; Record too short?
        beq     invalidRecord
        cmp     #$21            ; Record too long?
        bcs     invalidRecord
...
; We get here when bytesRead = byteCount
...
dataend:
        ...
        bcs     invalidRecord
okay1:
        eor     #$FF            ; Calculate 2's complement
        clc
        adc     #1
...
; We get here if we have a Record Type of 1
endofload:
        jsr     getHexByte      ; Get two hex digits (checksum)

;
; On the PAL-1/KIM-1, we have not UART/ACIA buffering, so we
; do a delay at the end just in case any spurour CR or LF
; characters come in. This is really ugly, brute force code.
...
; Changed bytes per line to hard coded 20

        lda     #$20             ; write bytesPerLine
...
nocarry1:
        ...
        cmp     #$20            ; Another hard coded change to bytes per line
        bne     writeLoop1      ; ...go back and loop

        lda     checksum        ; Calculate checksum 2's complement
        eor     #$ff
        clc
        adc     #1
...
; to stop.
;
; Prints CR to go back to start of line
;
; Registers changed: none
...
PromptToContinue:
        ...
        PHA
        JSR Imprint
        .asciiz "  <Space> to continue or <ESC> to stop"
@SpaceOrEscape:
        JSR GetKey
        CMP #' '
        BEQ @Cont
...
@rangeInvalid:
        JSR Imprint
        .byte "Error: Range's START must be before END", CR, LF, 0
        SEC
        RTS
@rangeOkay:
        CLC
...
.ifdef MINIASM
        .byte "A"
.endif
        .byte "BCDEFGHIJKMNORSUVW:=."
.elseif .defined(APPLE2)
        .byte "$?"
.ifdef MINIASM
        .byte "A"
.endif
        .byte "BCDFGHIJKNORSUVW:=."
.elseif .defined(OSI)
        .byte "$?"
.ifdef MINIASM
        .byte "A"
...
@loop:
        ...
        STA IN+1,X              ; Store character in buffer (skip first length byte)
        INX                     ; Advance index into buffer
        JSR PrintSpace          ; Space things out a bit W4JBM
        CPX #$7E                ; Buffer full?
...
WelcomeMessage:
        .byte CR,"PBUG for the PAL-1", CR, LF, 0

HelpString:
        .byte CR,LF,"Derived from JMON (C) by J.Tranter",CR,LF
        .byte "PAL-1 hardware by Liu Ganning",CR,LF
        .byte "Adaptation by Jim McClanahan W4JBM",CR,LF,CR,LF
.ifdef MINIASM
        .byte "Assemble    A <address>", CR, LF
.endif
        .byte "Breakpoint  B <n or ?> <address>", CR, LF
        .byte "Copy        C <start> <end> <dest>", CR, LF
        .byte "Dump        D <start>", CR, LF
        .byte "Fill        F <start> <end> <data>...", CR, LF
        .byte "Go          G <address>", CR, LF
        .byte "Write Hex   H <start> <end>", CR, LF
        .byte "Checksum    K <start> <end>",CR, LF
        .byte "Load Hex    L", CR, LF
        .byte "Info        N", CR, LF
        .byte "Registers   R", CR, LF
        .byte "Search      S <start> <end> <data>...", CR, LF
        .byte "Unassemble  U <start>", CR, LF
;        .byte "Verify      V <start> <end> <dest>", CR, LF
;        .byte "Write S rec W <start> <end> <go>",CR, LF
        .byte "Write       W <address> <data>...", CR, LF
        .byte "Monitor     $", CR, LF
        .byte "Trace       .", CR, LF
        .byte "Help        ?", CR, LF
        .byte 0
...
; This is in the RAM typically found on the RIOT chip for the PAL-1
