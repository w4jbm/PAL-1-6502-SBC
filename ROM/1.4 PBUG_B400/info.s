...
.endif
        ...
        JSR PrintCR
;
; Look for upper block of RAM on PAL-1
;
        LDA $1780   ; Load whatever is at $1780 and save
        PHA
        LDA #$00
        STA $1780
        LDA $1780
        ...
        LDA $17FF   ; Load whatever is at $17FF and save
        PHA
        LDA #$00
        STA $17FF
        LDA $17FF
        ...
        JSR Imprint           ; Print range of RAM
        .asciiz "RAM detected from: $1780 to $17FF"
        JSR PrintCR
; This is just a dummy push because we need to pop the
; top of stack in case no RAM was found.
        PHA

@NORIOT:
        PLA
        LDA $2000   ; Load whatever is at $1780 and save
        PHA
        LDA #$00
        STA $2000
        LDA $2000
        ...
        JSR Imprint           ; Print range of RAM
        .asciiz "RAM detected from: $2000 to $"
        JSR FTOPRAM2
        JSR PrintAddress
        JSR PrintCR
        PHA     ; another dummy push

@NOERAM:
        PLA

;
; This needs to be improved, but basically it looks to
; see if the timer seems to be running on a second RIOT
; to determine if the RIOT expansion board is present
; or not. --w4jbm
;

        JSR Imprint
        .asciiz "  2nd RIOT Board : "
...
; If $1704 seems to never change, we probably don't have
; a second RIOT installed.
...
FTOPRAM2:
        ...
        LDA #>$2000
        STA TOP+1
        JMP TORAMP2   ; Pass 2 at top of RAM starting from $2000
...
;  test will collide with the code being executed when writing to it.
        ...
        CMP #(>FindTopOfRAMEnd - 1)      ; Was FindTopOfRAM  ; Same page as this code?
        BEQ @Skip
