...
.elseif .defined(KIM1)
        .byte " ",CR
        .asciiz "? " ; The KIM always echos, if we did <ESC> at a prompt
                        ; the extra <CR> will fix things. *** JM
.endif
...
.endif
        JSR Imprint
.if .defined(APPLE1) .or .defined(APPLE2) .or .defined(KIM1) .or .defined(SBC)
        .byte "Invalid command. Type '?' for help", CR,LF, 0
.elseif .defined(OSI)
        .byte "Invalid command.", CR,LF, "Type '?' for help", CR,LF, 0
.endif
        RTS
...
; Display error that no CFFA1 is present.
        JSR Imprint
        .byte "No CFFA1 card found!", CR,LF, 0
        RTS
.endif
...
.endif
        ...
        JSR Imprint
        .byte "No ACI card found!", CR,LF, 0
        RTS
.endif
...
.endif
        ...
        JSR Imprint
        .byte "Woz Mon not found!", CR,LF, 0
        RTS
.elseif .defined(APPLE2)
...
.elseif .defined(OSI)
        .byte "CPU type 1-6502 2-65C02", CR,LF, "3-65816?", 0
.endif
@Retry2:
        JSR GetKey
        CMP #ESC
...
.endif
        JSR Imprint
.if .defined(APPLE1) .or .defined(APPLE2) .or .defined(KIM1) .or .defined(SBC)
        .byte "Error: start must be <= end", CR,LF, 0
.elseif .defined(OSI)
        .byte "Start must be <= end!", CR,LF, 0
.endif
        SEC
        RTS
@rangeOkay:
...
.elseif .defined(KIM1)
        .byte CR,LF,"JMON 1.3.6 by J. Tranter",CR,LF,0
.endif
...
.elseif .defined(OSI)
.ifdef MINIASM
        .byte "Assemble   A <a>", CR,LF
.endif
        .byte "Breakpoint B <n><a>", CR,LF
        .byte "Copy       C <s><e><d>", CR,LF
        .byte "Dump       D <s>", CR,LF
        .byte "Fill       F <s><e><d>.", CR,LF
        .byte "Go         G <a>", CR,LF
        .byte "Hex to dec H <a>", CR,LF
        .byte "BASIC      I", CR,LF
        .byte "Load Srec  J", CR,LF
        .byte "Checksum   K <s><e>",CR,LF
        .byte "Clr sCR,LFeen L", CR,LF
        .byte "Info       N", CR,LF
        .byte "Options    O", CR,LF
        .byte "Registers  R", CR,LF
        .byte "Search     S <s><e><d>.", CR,LF
        .byte "Test       T <s><e>", CR,LF
        .byte "Unassemble U <s>", CR,LF
        .byte "Verify     V <s><e><d>", CR,LF
        .byte "Write Srec W <st> <e> <g>",CR,LF
        .byte "Monitor    $", CR,LF
        .byte "Write      : <a><d>...", CR,LF
        .byte "Math       = <a>+/-<a>", CR,LF
        .byte "Trace      .", CR,LF
        .byte "Help       ?", CR,LF
        .byte 0
